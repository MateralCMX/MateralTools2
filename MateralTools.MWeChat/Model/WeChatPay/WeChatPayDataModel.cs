...
namespace MateralTools.MWeChat.WeChatPay
{
    public class WeChatPayDataModel
    {
        ...
        public string ToXml()
        {
            if (Data.Count > 0)
            {
                ...
                foreach (KeyValuePair<string, object> pair in Data)
                {
                    if (pair.Value != null)
                    {
                        ...
                        {
                            throw new MWeChatException("数据类型错误!只能为String或Int类型。");
                        }
                    }
                    else
                    {
                        throw new MWeChatException("含有值为null的字段!");
                    }
                }
                xml += "</xml>";
                return xml;
            ...
            {
                throw new MWeChatException("数据为空!");
            }
        }
        /// <summary>
        /// 将xml转为字典并返回对象内部的数据
        ...
        public SortedDictionary<string, object> FromXml(string xml, string key)
        {
            if (!xml.MIsNullOrEmpty())
            {
                ...
                if (Data["return_code"] is string dataValue && dataValue == "SUCCESS")
                {
                    if (!CheckSign(key))
                    {
                        throw new MWeChatException("签名错误");
                    }
                }
                return Data;
            }
            ...
            {
                throw new MWeChatException("待转换的XML为空");
            }
        }
        /// <summary>
        /// 字典转换URL参数
        ...
        public string ToUrlParams()
        {
            ...
            foreach (KeyValuePair<string, object> pair in Data)
            {
                if (pair.Value != null)
                {
                    ...
                    {
                        buff += string.Format("{0}={1}{2}", pair.Key, pair.Value, spacer);
                    }
                }
                else
                {
                    throw new MWeChatException("WxPayData内部含有值为null的字段!");
                }
            }
        ...
        {
            if (IsSet("sign") && !GetValue("sign").MIsNullOrEmptyStr())
            {
                string return_sign = GetValue("sign").ToString();
                return MakeSign(key) == return_sign;
            }
            else
            {
                return false;
